...
class FinanceNotifier extends StateNotifier<FinanceState> {
  ...
  FinanceNotifier(this._client) : super(FinanceState(
  ...
  Future<void> addTransaction(Map<String, dynamic> data) async {
    ...
    final account = state.accounts.firstWhere((a) => a.id == data['account_id']);
    final newBal = data['type'] == 'income'
      ? account.balance + (data['amount'] as num).toDouble()
      : account.balance - (data['amount'] as num).toDouble();
    await _client.from('accounts').update({'balance': newBal}).eq('id', account.id);

    state = state.copyWith(
      transactions: [tx, ...state.transactions],
      accounts: state.accounts.map((a) =>
        a.id == account.id
          ? Account(id: a.id, userId: a.userId, name: a.name,
              type: a.type, balance: newBal, color: a.color, isActive: a.isActive)
          : a
      ).toList(),
    );
  }
...
final financeProvider = StateNotifierProvider<FinanceNotifier, FinanceState>((ref) {
  ...
  final notifier = FinanceNotifier(client);
  notifier.fetchAll();
  return notifier;
});
