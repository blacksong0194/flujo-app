        ...
        {liquid.length > 0 && (
          <div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
              {liquid.map((acc) => (
                <AccountCard key={acc.id} account={acc} onEdit={handleOpenEdit} />
              ))}
            </div>
          </div>
        )}
        ...
        {debts.length > 0 && (
          <div className="card bg-brand-500/5 border border-brand-500/20">
            <p className="text-xs text-brand-400 flex items-start gap-2">
              ...
              <span>
                Las cuentas de <strong>deuda y préstamo que lleguen a $0</strong> se eliminarán automáticamente. Solo debes actualizar el saldo a 0 en la edición de la cuenta.
              </span>
            </p>
          </div>
        )}
